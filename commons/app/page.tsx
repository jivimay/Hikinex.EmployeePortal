...
function HomeView({ onEdit, apps, role, department, displayName, assignedIds, pinnedIds, canEdit, updates, navigate, onAdd, onRemove, onTogglePin, personalApps, onSavePersonalApps }: { onEdit?: (app: Application) => void; apps: Application[]; role: Role; department: string; displayName: string; assignedIds: Set<string>; pinnedIds: Set<string>; canEdit: boolean; updates: CompanyUpdate[]; navigate: (view: View) => void; onAdd: (app: Application) => void; onRemove: (app: Application) => void; onTogglePin: (app: Application) => void; personalApps: PersonalApp[]; onSavePersonalApps: (items: PersonalApp[]) => Promise<boolean> }) {
  ...
  return <main className={`portal commons launchpad ${dark ? "dark" : ""}`}>
    ...
    <section className="main">
      ...
      <div className="content">
        ...
        {view === "Home" && <HomeView onEdit={canManageApps ? openAppEditor : undefined} apps={apps} role={role} department={department} displayName={identity?.greetingName ?? ""} assignedIds={assignedIds} pinnedIds={pinnedIdSet} canEdit={canEdit} updates={companyUpdates} navigate={navigate} onAdd={addApp} onRemove={removeApp} onTogglePin={togglePin} personalApps={personalApps} onSavePersonalApps={savePersonalApps} />}
