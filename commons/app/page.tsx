...
function PersonalApps({ items, onSave, search }: { items: PersonalApp[]; onSave: (items: PersonalApp[]) => Promise<boolean>; search: string }) {
  ...
  return <section className="personal-section"><div className="section-head"><div><h2>Made yours</h2><small>Your personal apps, in your own space.</small></div><div className="personal-section-actions"><small>Only you</small>{items.length > 0 && <button className="personal-pin-all" disabled={busy} onClick={async () => { setBusy(true); await onSave(items.map(app => ({ ...app, pinned: !items.every(item => item.pinned) }))); setBusy(false); }}><PinIcon />{items.every(item => item.pinned) ? "Unpin all" : "Pin all apps"}</button>}</div></div><div className="personal-grid">{[...items].sort((a, b) => Number(Boolean(b.pinned)) - Number(Boolean(a.pinned))).filter((item) => `${item.name} ${item.url}`.toLowerCase().includes(search.toLowerCase())).map((item) => <article className="personal-tile" key={item.id}><a href={item.url} target="_blank" rel="noopener noreferrer"><span className="app-icon">{item.name.charAt(0).toUpperCase()}</span><span><strong>{item.name}</strong><small>{new URL(item.url).hostname}</small></span></a><button className="personal-pin" type="button" aria-pressed={Boolean(item.pinned)} aria-label={`${item.pinned ? "Unpin" : "Pin"} ${item.name} on the dashboard`} disabled={busy} onClick={async () => { setBusy(true); await onSave(items.map(app => app.id === item.id ? { ...app, pinned: !app.pinned } : app)); setBusy(false); }}><PinIcon /><span>{item.pinned ? "Pinned" : "Pin"}</span></button><button type="button" aria-label={`Remove ${item.name}`} disabled={busy} onClick={async () => { setBusy(true); await onSave(items.filter((app) => app.id !== item.id)); setBusy(false); }}>×</button></article>)}<button className="personal-add" onClick={() => { setOpen(true); setError(""); }}>＋ Add your own app</button></div>{open && <form className="personal-form" onSubmit={save}><h3>Add your own app</h3><p>A personal shortcut. Access is managed by the destination app.</p><div><label>App name<input ref={input} value={name} onChange={(event) => setName(event.target.value)} required maxLength={40} /></label><label>Website address<input type="url" value={url} onChange={(event) => setUrl(event.target.value)} placeholder="https://" required maxLength={2048} /></label></div>{error && <p role="alert">{error}</p>}<button className="primary" disabled={busy}>{busy ? "Saving…" : "Add to my apps"}</button><button type="button" disabled={busy} onClick={() => setOpen(false)}>Cancel</button></form>}</section>;
}
...
function AppTile({ onEdit, app, assigned, protectedApp, directory = false, canEdit, pinned = false, onAdd, onAddAndPin, onRemove, onTogglePin }: { onEdit?: (app: Application) => void; app: Application; assigned: boolean; protectedApp: boolean; directory?: boolean; canEdit: boolean; pinned?: boolean; onAdd: (app: Application) => void; onAddAndPin?: (app: Application) => void; onRemove: (app: Application) => void; onTogglePin?: (app: Application) => void }) {
  return <article className={`app-tile ${assigned ? "assigned" : ""} ${pinned ? "pinned-app" : ""}`}>
    {assigned ? <a className="app-card-link" href={app.url} target="_blank" rel="noopener noreferrer" aria-label={`Open ${app.name} in a new tab`}><AppLogo app={app} /><span className="app-copy"><strong>{app.name}</strong><small>{app.description}</small></span></a> : <div className="app-card-content"><AppLogo app={app} /><span className="app-copy"><strong>{app.name}</strong><small>{app.description}</small></span></div>}
...
function HomeView({ apps, role, department, displayName, assignedIds, pinnedIds, canEdit, updates, navigate, onAdd, onRemove, onTogglePin, personalApps, onSavePersonalApps }: { apps: Application[]; role: Role; department: string; displayName: string; assignedIds: Set<string>; pinnedIds: Set<string>; canEdit: boolean; updates: CompanyUpdate[]; navigate: (view: View) => void; onAdd: (app: Application) => void; onRemove: (app: Application) => void; onTogglePin: (app: Application) => void; personalApps: PersonalApp[]; onSavePersonalApps: (items: PersonalApp[]) => Promise<boolean> }) {
  ...
  return <>
    ...
    <section className="work-section"><div className="section-head"><div><h2>Your work essentials</h2><small>My pinned apps · Selected for your role. Organized by you.</small></div><button onClick={() => navigate("Apps")}>App directory</button></div><div className="apps-grid">{visibleApps.map((app) => <AppTile key={app.id} app={app} assigned protectedApp={defaultAppIds(role, department).includes(app.id)} pinned canEdit={canEdit} onAdd={onAdd} onRemove={onRemove} onTogglePin={onTogglePin} />)}{personalApps.filter(item => item.pinned && canUseShortcut(item.url, role, department) && `${item.name} ${item.url}`.toLowerCase().includes(search.toLowerCase())).map(item => <article className="app-tile pinned-app" key={item.id}><a className="app-card-link" href={item.url} target="_blank" rel="noopener noreferrer"><span className="app-icon">{item.name.charAt(0).toUpperCase()}</span><span className="app-copy"><strong>{item.name}</strong><small>Personal app</small></span></a><div className="app-actions"><button className="pin-app" aria-pressed="true" aria-label={`Unpin ${item.name} on the dashboard`} onClick={() => onSavePersonalApps(personalApps.map(app => app.id === item.id ? { ...app, pinned: false } : app))}><PinIcon /></button></div></article>)}{!visibleApps.length && !personalApps.some(item => item.pinned && canUseShortcut(item.url, role, department)) && <p className="app-empty">{search ? "No matching work apps." : "Pin an app from the directory to get started."}</p>}<button className="request-card" onClick={() => navigate("Apps")}><span>＋</span><strong>Add or pin an App</strong><small>Browse your directory</small></button></div></section>
    <PersonalApps items={personalApps.filter((item) => canUseShortcut(item.url, role, department))} onSave={onSavePersonalApps} search={search} />
...
function AppsView({ onEdit, apps, role, department, assignedIds, pinnedIds, canEdit, navigate, onAdd, onAddAndPin, onRemove, onTogglePin }: { onEdit?: (app: Application) => void; apps: Application[]; role: Role; department: string; assignedIds: Set<string>; pinnedIds: Set<string>; canEdit: boolean; navigate: (view: View) => void; onAdd: (app: Application) => void; onAddAndPin: (app: Application) => void; onRemove: (app: Application) => void; onTogglePin: (app: Application) => void }) {
  ...
  return <section><PageHead eyebrow="WORK" title="Add an App" copy={canEdit ? `Search the applications approved for the ${role} role. Use the pin to add an app directly to Quick Access.` : "Explore the approved directory in Review mode. Sign in to save applications to Quick Access."} />
    ...
    <div className="directory-tools"><label aria-label="Search apps by name or category">⌕ <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search apps by name or category" /></label><div>{groups.map((group) => <button className={selected === group ? "active" : ""} onClick={() => setSelected(group)} key={group} aria-pressed={selected === group}>{group}<span>{groupCount(group)}</span></button>)}</div></div><div className="directory-grid">{results.map((app) => <AppTile onEdit={onEdit} key={app.id} app={app} assigned={assignedIds.has(app.id)} protectedApp={defaultAppIds(role, department).includes(app.id)} directory pinned={pinnedIds.has(app.id)} canEdit={canEdit} onAdd={onAdd} onAddAndPin={onAddAndPin} onRemove={onRemove} onTogglePin={onTogglePin} />)}</div>{results.length === 0 && <div className="empty-state"><strong>No applications found</strong><span>Try another name or category.</span></div>}</section>;
}
...
function AnnouncementsView({ role, department, items, onCreate, notify }: { role: Role; department: string; items: CompanyUpdate[]; onCreate: (draft: CompanyUpdateDraft) => Promise<boolean>; notify: (message: string) => void }) {
  ...
  const [draft, setDraft] = useState<CompanyUpdateDraft>({ title: "", summary: "", body: "", audience: canPublishCompanyWide(role, department) ? "company" : "department", department: canPublishCompanyWide(role, department) ? null : department });
  const publish = async () => {
...
export default function Portal() {
  ...
  const assignedIds = useMemo(() => new Set([...defaultAppIds(role, department), ...addedIds.filter((id) => catalogAppIds(role, department).includes(id))]), [role, department, addedIds]);
  ...
  const savePersonalApps = async (items: PersonalApp[]) => {
    ...
    if (validated.length !== items.length || validated.some((item) => !canUseShortcut(item.url, role, department))) { notify("Some app details are invalid."); return false; }
  ...
  const addApp = async (app: Application) => {
    if (!session || !supabase) return;
    if (assignedIds.has(app.id) || !catalogAppIds(role, department).includes(app.id)) return;
  ...
  const removeApp = async (app: Application) => {
    if (!session || !supabase || defaultAppIds(role, department).includes(app.id)) return;
  ...
  const addAndPin = async (app: Application) => {
    if (!catalogAppIds(role, department).includes(app.id)) return;
  ...
  const createCompanyUpdate = async (draft: CompanyUpdateDraft) => {
    if (!session || !supabase || !canPublishUpdates(role, department)) return false;
    const payload = {
      ...draft,
      audience: canPublishCompanyWide(role, department) ? draft.audience : "department",
      department: !canPublishCompanyWide(role, department) || draft.audience === "department" ? department : null,
  ...
  return <main className={`portal commons launchpad ${dark ? "dark" : ""}`}>
